 ...
 * Covers deny-by-default gating, gate-open registration, always-on
...
describe('isAllowed', () => {
  ...
  it('permits ordinary read tools and ui_evaluate regardless of gate state', () => {
    assert.equal(isAllowed('quote_get', {}), true);
    assert.equal(isAllowed('data_get_ohlcv', { TV_ALLOW_DANGEROUS: '1' }), true);
    assert.equal(isAllowed('ui_evaluate', {}), true);
    assert.equal(isAllowed('ui_evaluate', { TV_ALLOW_DANGEROUS: '1' }), true);
  });
});
...
describe('wrapRegistrar', () => {
  ...
  it('registers ui_evaluate without a special env gate', () => {
    const server = wrapRegistrar(stubServer(), { env: {}, log: stubLog() });
    server.tool('ui_evaluate', 'desc', {}, async () => {});
    assert.deepEqual(server.registered, ['ui_evaluate']);
  });
