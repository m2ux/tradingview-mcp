...
export function registerCaptureTools(server) {
  server.tool('capture_screenshot', 'Take a screenshot of the TradingView chart', {
    ...
    target: z.string().optional().describe('Target tab: chart_id, URL substring, CDP target id, or a saved LAYOUT/tab name (e.g. "OIL_IG"; use "layout:<name>" to force layout-name matching). Captures this tab without switching the active tab. Omit for the attached tab. Names come from tab_list (layout_name). If the response has "retryable": true, TradingView is momentarily busy — wait ~1s and retry.'),
  ...
  server.tool('capture_snapshot', 'Capture a full snapshot of the currently displayed chart, headlessly: visible time & price range, OHLCV over the visible bars, active studies (with per-bar series aligned to the visible window), user drawings, Pine graphics (lines/labels/tables/boxes), and a screenshot. One call instead of the multi-tool "Analyze my chart" sequence.', {
    ...
    target: z.string().optional().describe('Target tab: chart_id, URL substring, CDP target id, or a saved LAYOUT/tab name (e.g. "OIL_IG"; use "layout:<name>" to force layout-name matching). Snapshots this tab without switching the active tab. Omit for the attached tab. Names come from tab_list (layout_name). If the response has "retryable": true, TradingView is momentarily busy — wait ~1s and retry.'),
