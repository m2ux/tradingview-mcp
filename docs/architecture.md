...
*(Tool counts are the always-on surface. The five gated power tools — `tv_update`, `tv_launch`, `alert_delete`, `draw_clear`, `batch_run` — register only on `TV_ALLOW_DANGEROUS=1`. `ui_evaluate` is always on.)*
